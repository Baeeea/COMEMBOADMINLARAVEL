...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  ...
  <a class="nav-link d-flex align-items-center"
    ...
    <span class="fs-5 lead text-light">K. Anderson</span>
  ...
  <ul class="collapse list-unstyled ps-4 bg-secondary" id="profileDropdown">
    <li class="sidebar-item">
      <a href="{{ route('profile') }}" class="sidebar-link text-light">
            ...
            <div class="table-responsive">
              <table class="table table-hover rounded-4 overflow-hidden shadow">
                ...
                <tbody>
                  ...
                  <tr>
                    <td class="py-4">{{ $announcement->title }}</td>
                    <td class="py-4">{{ date('m / d / Y', strtotime($announcement->createdAt)) }}</td>
                    <td class="py-4">
                      <a href="{{ route('announcements.edit', $announcement->id) }}" class="btn btn-primary btn-sm px-3 me-2">Edit</a>
                      <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                              onclick="setDeleteForm('{{ route('announcements.destroy', $announcement->id) }}')">Delete</button>
                    </td>
                  </tr>
                  @empty
                  <tr>
            ...
            <div class="modal-body">
              <form method="POST" action="{{ route('announcements.store') }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control" id="description" name="description" rows="8" placeholder="Enter description" required></textarea>
                </div>
                <div class="row">
